...
def forecast_figure(forecast: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.text(
        0.5,
        0.002,
        "Prophet and XGBoost are numerically tied on Electricity at four decimal places.",
        ha="center",
        va="bottom",
        fontsize=7.5,
    )
...
def ablation_figure(ablation: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.set_yticklabels(ablation["dataset"].map(DISPLAY_NAMES))
    ax.invert_yaxis()
    ...
    for i, row in enumerate(ablation.itertuples(index=False)):
        ax.text(
            ...
            f"(Holm-adjusted p={row.primary_rmse_holm_across_datasets:.4f})",
            va="center",
            fontsize=8,
        )
...
def detector_figure(macro: pd.DataFrame, outdir: Path) -> None:
    ...
    ax.legend(
        title="Dataset",
        labels=[display_dataset(name) for name in pivot.columns],
        ncol=3,
        loc="upper left",
    )
...
def latency_figure(resources: pd.DataFrame, outdir: Path) -> None:
    ...
    fig.text(
        0.5,
        0.002,
        "ARIMA and Prophet include per-step refitting; the other models use fixed fitted predictors.",
        ha="center",
        va="bottom",
        fontsize=7.5,
    )
...
def write_ablation_table(ablation: pd.DataFrame, outdir: Path) -> None:
    ...
    for row in ablation.itertuples(index=False):
        ...
        line = (
            f"{display_dataset(row.dataset)} & {core} & {linear} & "
            f"{row.mean_relative_improvement_percent:.1f}\\% & "
...
def main() -> None:
    ...
    metadata = {
        "final_polish": [
            "Corrected ILINet capitalization",
            "Moved Electricity tie statement to a figure-level note",
            "Added grayscale-friendly hatch patterns to detector bars",
            "Clarified that ARIMA and Prophet include per-step refitting",
            "Used Holm-adjusted p wording in the primary ablation figure",
        ],
        "main_figures": [
    ...
    print("===== Final manuscript assets completed =====")
    print(
        ablation[
            [
                "dataset",
